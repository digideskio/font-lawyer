...
function fl_get_fontfile( $fontname ) {
 return flres(flvault.'/'.$fontname.'.ttf');
}
...
function fl_load_sheet( $sheet_id ) {
 ...
 if ( !file_exists( vault.$font['name'].".ttf" ) )
 fllog('Warning: font sprites loaded for unavailable font file '.$font['name'].'.ttf');
 return $sprites;
}
...
function fl_sprite( $_file, $sheet_id, $sprite_id, $tag='div', $output_filetype="png" ) {
 ...
 if ( count($descriptions) == 0 ) {
  fllog('fl_sprite: no descriptions ('.flcache.'/'.$sheet_id.'.txt'
   .') available for sheet_id `'.$sheet_id.'`, so nothing done FILE: `'.$_file.'`');
  return '<!-- FontLawyer: sprite description not yet available, place in cache dir as '
   .$sheet_id.'.txt; see docs for details on how to make this -->';
 }
 $render=false;
 ...
 return '<'.$tag.' class="'.$sheet_id.'-'.$sprite_id.'"></'.$tag.'>';
}
